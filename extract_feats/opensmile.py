...
def get_feature_opensmile(config, filepath: str) -> list:
    ...
    print("Opensmile config path: ", opensmile_config_path)
    exe_path = os.path.join(config.opensmile_path,'bin','Win32')
    # Opensmile 命令
    cmd = 'cd ' + exe_path + ' && SMILExtract_ReleasePortaudio.exe -C ' + opensmile_config_path + ' -I ' + filepath + ' -O ' + single_feat_path + ' -appendarff 0'
