...
class LSTM(DNN):
    ...
    def reshape_input(self, data: np.ndarray) -> np.ndarray:
        ...
        model.add(Bidirectional(KERAS_LSTM(rnn_size, input_shape=(1, input_shape))))  # (time_steps = 1, n_feats)
        model.add(Attention())  # Add Attention layer
        model.add(Bidirectional(KERAS_LSTM(rnn_size, return_sequences=True)))  # Add Bidirectional LSTM layer
        model.add(Dropout(dropout))
        model.add(Dense(hidden_size, activation='relu'))
        model.add(Dense(n_classes, activation='softmax'))  # 分类层
